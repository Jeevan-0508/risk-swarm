...
export const COMPARATIVE_WORDS = [
  'more', 'less', 'better', 'worse', 'bigger', 'smaller', 'larger', 'higher', 'lower', 'greater',
  'healthier', 'safer', 'stronger', 'weaker', 'faster', 'slower', 'cheaper',
  'largest', 'biggest', 'smallest', 'highest', 'lowest', 'greatest', 'most', 'least',
  'win', 'wins', 'won', 'winner', 'beat', 'beats', 'defeat', 'defeats',
];
...
const INTENT_RULES: Array<[Intent, RegExp]> = [
  ...
  ['comparison', new RegExp(
    '\\b(vs\\.?|versus|compared? (?:to|with)|compar(?:e|es|ing)|difference between|differs? from|how does .+ differ|' +
      '(?:which|who|what) (?:\\S+\\s+){0,2}(?:is|are|has|have|would|will) (?:' + COMPARATIVE_WORDS.join('|') + '))\\b',
  )],
